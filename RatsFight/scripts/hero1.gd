...
#			_parent.change_state(globals.STATE.WALK)
		...
		# Switch to SLIDE
			...
			_parent.change_state(globals.STATE.SLIDE)
		# Switch to SPECIAL (SPECIAL_SETUP)
		elif _parent.input_special_just_pressed() && _parent.can_special():
			_parent.change_state(globals.STATE.SPECIAL_SETUP)
...
class SpecialSetupState:
	...
	func update(delta):
		if _parent._special_setup:
			...
			_parent._node_timer_special.start()
...
class SpecialStepOneState:
	...
	func start():
		_parent._speed = SPECIAL_SPEED

	func update(delta):
		if _parent.input_jump_just_pressed() && _parent._touch_floor:
			_parent._touch_floor = false
			_parent._velocity.y = -JUMP_FORCE
		_parent._new_left = null
		if _parent._input_walk_left:
			_parent._new_left = 1
		elif _parent._input_walk_right:
			_parent._new_left = -1
		if _parent._new_left != null && _parent._current_left != _parent._new_left:
			_parent._current_left = _parent._new_left
			_parent.set_scale(Vector2(_parent._current_left, 1))
		_parent._velocity.x = 0 if _parent._new_left == null else -_parent._speed * _parent._current_left
		_parent.move_body(delta)
		_parent.check_items_to_consume()
...
func _init():
	...
	_states = {
		...
		globals.STATE.SLIDE: SlideState.new(self)
	}
...
func _on_timer_special_timeout():
	_special_finished = true
